...
predicate TimestampedRslNextOneReplica(ps:TimestampedRslState, ps':TimestampedRslState, idx:int, ios:seq<TimestampedLIoOp<NodeIdentity, RslMessage>>)
{
  RslNextOneReplica(UntimestampRslState(ps), UntimestampRslState(ps'), idx, UntagLIoOpSeq(ios))
    ...
    (if |ios| > 0 && ios[0].LIoOpReceive? then
      && ios[0] in ios
      && TimeLe(ps'.t_replicas[idx].ts, Rsl_RecvPerfUpdate(ps.t_replicas[idx].ts, ios[0].r.msg.ts, hstep))
      && (ios[0].r.msg.ts <= ps.t_replicas[idx].ts + Timeout())
    else if |ios| > 0 && ios[0].LIoOpTimeoutReceive? then
      && TimeLe(ps'.t_replicas[idx].ts, Rsl_TimeoutPerfUpdate(ps.t_replicas[idx].ts, hstep))
    else
      && TimeLe(ps'.t_replicas[idx].ts, Rsl_NoRecvPerfUpdate(ps.t_replicas[idx].ts, hstep))
      )

      && (forall io :: io in ios && io.LIoOpSend? ==>
          TimeLe(io.s.msg.ts, ps'.t_replicas[idx].ts + D)
      )
}
