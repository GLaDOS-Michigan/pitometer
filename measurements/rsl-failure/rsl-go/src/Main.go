...
// <node_1 ip> <node_1 port> ... <node_n ip> <node_n port> <my ip> <my port> <clock_on> <inet> <duration> <batch_size> <failureCountdown>
func main() {

	fmt.Printf("%v\n", os.Args)

	var failureCountdown, err0 = strconv.ParseInt(os.Args[len(os.Args)-1], 10, 64)
	if int(failureCountdown) < 0 || err0 != nil {
	...
	var inet_str = os.Args[len(os.Args)-4]

	// Grab the fourth-to-last argument from os.Args -- that is whether to turn on logging
	var clock_mode = os.Args[len(os.Args)-5]
	switch clock_mode {
	case "on":
		clock.ON = true
	case "off":
		clock.ON = false
	default:
		fmt.Printf("Error: Invalid clock mode %v\n", os.Args[len(os.Args)-4])
		os.Exit(1)
	}

	os.Args = os.Args[:len(os.Args)-5]
	clock.INIT_SIZE = INIT_SIZE
