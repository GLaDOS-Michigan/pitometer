...
func (_this CLearnerTuple) String() string {
	...
	case CLearnerTuple_CLearnerTuple:
		{
			return "CLearnerTuple.CLearnerTuple" + "(" + _dafny.String(data.Received__2b__message__senders) + ", " + _dafny.String(data.Candidate__learned__value) + ")"
		}
	default:
		{
			return "<unexpected>"
...
func (_this CLearnerState) String() string {
	...
	case CLearnerState_CLearnerState:
		{
			return "CLearnerState.CLearnerState" + "(" + _dafny.String(data.Rcs) + ", " + _dafny.String(data.Max__ballot__seen) + ", " + _dafny.String(data.Unexecuted__ops) + ", " + _dafny.String(data.SendDecision) + ", " + _dafny.String(data.Opn) + ", " + _dafny.String(data.Recv2b) + ", " + _dafny.String(data.RecvCp) + ")"
		}
	default:
		{
			return "<unexpected>"
...
func (_this *CompanionStruct_Default___) LearnerState__Init(rcs _245_LiveRSL____ReplicaConstantsState__i_Compile.ReplicaConstantsState) CLearnerState {
	goto TAIL_CALL_START
TAIL_CALL_START:
	var learner CLearnerState = Type_CLearnerState_().Default().(CLearnerState)
	var _ = learner
	var _4741_endPoint _9_Native____Io__s_Compile.EndPoint
	var _ = _4741_endPoint
	_4741_endPoint = ((((rcs).Dtor_all()).Dtor_config()).Dtor_replica__ids()).IndexUint((rcs).Dtor_my__index()).(_9_Native____Io__s_Compile.EndPoint)
