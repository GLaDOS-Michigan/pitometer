...
module RslPhase2Proof_PostFail_Helper0 {
...
{   
    ...
    AlwaysInvariantP2_Maintained_ViewSuspectors(ts, ts', opn);
    assert  || ts'.t_replicas[1].v.replica.proposer.election_state.current_view_suspectors == {}
...
lemma AlwaysInvariantP2_Maintained_ViewSuspectors(ts:TimestampedRslState, ts':TimestampedRslState, opn:OperationNumber) 
    requires CommonAssumptions(ts) && CommonAssumptions(ts')
    requires P2Assumption(ts, opn) 
    requires InPhase2(ts') ==> P2Assumption(ts', opn)
    requires TimestampedRslNext(ts, ts')
...
lemma  AlwaysInvariantP2_Maintained_BatchSize2a(ts:TimestampedRslState, ts':TimestampedRslState, opn:OperationNumber) 
    requires CommonAssumptions(ts) && CommonAssumptions(ts')
    requires P2Assumption(ts, opn) 
    requires InPhase2(ts') ==> P2Assumption(ts', opn)
    requires TimestampedRslNext(ts, ts')
