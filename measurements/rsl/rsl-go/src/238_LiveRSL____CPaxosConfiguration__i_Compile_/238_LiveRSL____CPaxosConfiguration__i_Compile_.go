...
func (_this *CompanionStruct_Default___) PaxosEndPointIsValid(endPoint _9_Native____Io__s_Compile.EndPoint, config CPaxosConfiguration) bool {
	return _180_Common____UdpClient__i_Compile.Companion_Default___.EndPointIsValidIPV4(endPoint)
}
func (_this *CompanionStruct_Default___) CGetReplicaIndex(replica _9_Native____Io__s_Compile.EndPoint, config CPaxosConfiguration) (bool, uint64) {
	...
	for (_4730_i) < (uint64(((config).Dtor_replica__ids()).CardinalityUint64())) {
		if (replica).Equals(((config).Dtor_replica__ids()).IndexUint(_4730_i).(_9_Native____Io__s_Compile.EndPoint)) {
			found = true
			index = _4730_i
			{
			}
		...
		if (_4730_i) == ((uint64(((config).Dtor_replica__ids()).CardinalityUint64())) - (func() uint64 { return (uint64(1)) })()) {
			found = false
			return found, index
		}
