...
lemma RSLTopLevel_PostFailover(tglb:seq<TimestampedRslState>, opn:OperationNumber)
    requires |tglb| > 0
    ...
    requires P1_def.Phase1Invariant(tglb[0], opn)
    requires forall i | 0 <= i < |tglb| :: RslAssumption(tglb[i], opn)
    ensures forall j | 0 <= j < |tglb| :: PerformanceGuarantee_Final(tglb[j])
{
    var P2_idx := P1_top.Phase1TopLevel(tglb, opn);
    if P2_idx >= |tglb| {
        return;        
    } else {
        assert forall i | 0 <= i < P2_idx :: |tglb[i].t_replicas| > 2;
