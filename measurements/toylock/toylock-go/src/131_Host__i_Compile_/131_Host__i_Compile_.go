...
func (_this CScheduler) String() string {
	...
	case CScheduler_CScheduler:
		{
			return "CScheduler.CScheduler" + "(" + _dafny.String(data.Node__impl) + ")"
		}
	default:
		{
			return "<unexpected>"
...
func (_this *CompanionStruct_Default___) HostInitImpl() (bool, CScheduler, _dafny.Seq, _9_Native____Io__s_Compile.EndPoint) {
...
TAIL_CALL_START:
	...
	id = (config).IndexUint64(_1747_my__index).(_9_Native____Io__s_Compile.EndPoint)
...
func (_this *CompanionStruct_Default___) HostNextImpl(host__state CScheduler, delay int, nodeGrantCounter *clock.Counter, nodeAcceptLog *clock.Stopwatch) (bool, CScheduler, string) {
	var ok bool = false
	var _ = ok
	...
	var _ = _out105
	var status string
	_out105, status = ((host__state).Dtor_node__impl()).HostNextMain(delay, nodeGrantCounter, nodeAcceptLog)
	_1749_okay = _out105
	if _1749_okay {
		{
		}
	...
	ok = _1749_okay
	return ok, host__state_k, status
}
